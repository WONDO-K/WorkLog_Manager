...
test('일일 요약 초안 저장, 검토 확정, 수정 충돌 및 재접속 보존',async()=>{
 const dir=mkdtempSync(join(tmpdir(),'wm-daily-'));const path=join(dir,'daily.sqlite'),exportDir=join(dir,'exports');
 let db=openStore(path);const server=createApp(db,{companyName:'테스트',exportDir});await new Promise(r=>server.listen(0,'127.0.0.1',r));
 ...
 try{
  ...
  assert.equal(existsSync(join(exportDir,'2026-09-09.md')),false);
  ...
  assert.equal(readFileSync(join(exportDir,'2026-09-09.internal.md'),'utf8'),'# 내부');
  assert.equal(readFileSync(join(exportDir,'2026-09-09.md'),'utf8'),'# 외부');
